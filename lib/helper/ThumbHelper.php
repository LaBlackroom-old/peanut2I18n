 ...
 *
 *
 * description: ces fonctions doivent étre appeler lors de l'affichage d'une image sur le site.
 * on génére des thumb é partir de l'image uploadée ou d'une image par défaut le cas échéant.
 * pour pouvoir l'utiliser il faut:
 *  - créer un répertoire dans uploads en par exemple du nom du Model qui é l'image.
 *  - dans ce répertoire créer deux sous répertoires soruce et thumb avec les droits d'accés
 *  - uploader les images dansle répertoire source.
 * lors de l'affiche un fichier du méme nom que source préfixé par la taille demandée
 ...
 * uploads/user/source/ -> lé ou on doit uploader le images
 * uploads/user/thumb/
 *
 ...
 */
  ...
  */
   ...
   * @return <image_path>
  */
  function doThumb_watermark($image_name, $folder, $options = array(), $resize = 'scale', $default = 'default.jpg', $watermark = 'images/watermark.png', $position = 'middle-center')
  {
    //valeur par défaut si elles ne sont pas définies
    if(!isset($options['width']))
      $options['width'] = 50;
    ...
    if(!is_file(sfConfig::get('sf_web_dir').'/'.$new_img) OR filemtime($source)>filemtime($new_img))
    {
      ...
      $img->thumbnail($options['width'],$options['height'],$resize)
          ->overlay(new sfImage($watermark), $position)
          ->saveAs($new_img);
    }
